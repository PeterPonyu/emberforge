...
impl SystemPromptBuilder {
    ...
    pub fn build(&self) -> Vec<String> {
        ...
        sections.push(get_tool_usage_section());
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        ...
        for candidate in [
            dir.join("EMBER.md"),
            dir.join("EMBER.local.md"),
            dir.join(".ember").join("EMBER.md"),
            dir.join(".ember").join("instructions.md"),
            dir.join("CLAW.md"),
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Emberforge settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
fn get_tool_usage_section() -> String {
    let items = prepend_bullets(vec![
        "When the user asks about files, code, or the workspace, USE tools (bash, read_file, glob_search, grep_search) to get real data instead of guessing.".to_string(),
        "Never invent a file path or repository artifact (for example `status.md`, `todo.md`, or `src/`) unless it already appears in the prompt/context or you discovered it with a tool.".to_string(),
        "When the user asks you to run a command, USE the bash tool. Do NOT just print the command.".to_string(),
        "When the user asks to edit or create files, USE write_file or edit_file tools. Do NOT just show the code.".to_string(),
        "If a file/path tool call fails or a search returns no matches, do not stop and do not give generic troubleshooting steps to the user. Keep working: broaden the search, inspect the workspace, or use bash/git to gather the missing context.".to_string(),
        "For project or repository status requests, prefer the git status snapshot already in context or use bash with `git status --short --branch` / `git diff` instead of guessing a `status.md` file.".to_string(),
        "For simple conversational questions (greetings, explanations, opinions), respond directly WITHOUT tools.".to_string(),
        "If you need to search the web, USE WebSearch. If you need to fetch a URL, USE WebFetch.".to_string(),
        "Always prefer using tools over describing what you would do.".to_string(),
    ]);
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        // pid + atomic counter prevents collisions on macOS, where
        // SystemTime resolution is microseconds and parallel tests can
        // otherwise share the same root.
    ...
    #[test]
    fn tool_usage_section_requires_recovery_instead_of_guessing() {
        let section = get_tool_usage_section();

        assert!(section.contains("Never invent a file path or repository artifact"));
        assert!(section.contains("If a file/path tool call fails or a search returns no matches"));
        assert!(section.contains("git status --short --branch"));
        assert!(section.contains("status.md"));
    }
