                ...
                let result_message = match permission_outcome {
                    PermissionOutcome::Allow => {
                        ...
                        if pre_hook_result.is_denied() {
                            ...
                            let mut halt_after_tool_result = false;
                            let (mut output, mut is_error) =
                                match self.tool_executor.execute(&tool_name, &input) {
                                    Ok(output) => (output, false),
                                    Err(error) => {
                                        halt_after_tool_result = error.is_fatal();
...
mod tests {
    ...
    #[test]
    fn fatal_tool_errors_stop_conversation_after_recording_tool_result() {
        struct SingleCallApiClient {
            calls: usize,
        }
        ...
        let summary = runtime
            .run_turn("use the stopper tool", None)
            .expect("fatal tool stop should return a partial summary");
        ...
        assert!(matches!(
            ...
            ContentBlock::ToolResult { is_error: true, output, .. }
                if output.contains("stop requested")
        ));
    }
