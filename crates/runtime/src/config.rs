...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum RuntimeUiBannerVariant {
    #[default]
    Auto,
    Compact,
    Wide,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RuntimeUiBannerConfig {
    mode: RuntimeUiBannerMode,
    variant: RuntimeUiBannerVariant,
}

impl Default for RuntimeUiBannerConfig {
    fn default() -> Self {
        Self {
            mode: RuntimeUiBannerMode::Auto,
            variant: RuntimeUiBannerVariant::Auto,
        }
    }
}

impl RuntimeUiBannerConfig {
    #[must_use]
    pub fn new(mode: RuntimeUiBannerMode, variant: RuntimeUiBannerVariant) -> Self {
        Self { mode, variant }
    }

    #[must_use]
    pub fn mode(&self) -> RuntimeUiBannerMode {
        self.mode
    }

    #[must_use]
    pub fn variant(&self) -> RuntimeUiBannerVariant {
        self.variant
    }
}

#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct RuntimeUiMotionConfig {
    reduced: bool,
}

impl RuntimeUiMotionConfig {
    #[must_use]
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum RuntimeUiAnimationMode {
    Off,
    Intro,
    #[default]
    Auto,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RuntimeUiAnimationConfig {
    mode: RuntimeUiAnimationMode,
}

impl Default for RuntimeUiAnimationConfig {
    fn default() -> Self {
        Self {
            mode: RuntimeUiAnimationMode::Auto,
        }
    }
}

impl RuntimeUiAnimationConfig {
    #[must_use]
    pub fn new(mode: RuntimeUiAnimationMode) -> Self {
        Self { mode }
    }

    #[must_use]
    pub fn mode(&self) -> RuntimeUiAnimationMode {
        self.mode
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum RuntimeUiHudPreset {
    Off,
    #[default]
    Minimal,
    Focused,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RuntimeUiHudConfig {
    preset: RuntimeUiHudPreset,
}

impl Default for RuntimeUiHudConfig {
    fn default() -> Self {
        Self {
            preset: RuntimeUiHudPreset::Minimal,
        }
    }
}

impl RuntimeUiHudConfig {
    #[must_use]
    pub fn new(preset: RuntimeUiHudPreset) -> Self {
        Self { preset }
    }

    #[must_use]
    pub fn preset(&self) -> RuntimeUiHudPreset {
        self.preset
    }
}
...
impl EffortLevel {
    ...
    pub fn parse(value: &str) -> Option<Self> {
        match value.to_ascii_lowercase().as_str() {
            ...
            "thorough" | "high" | "deep" => Some(Self::Thorough),
            _ => None,
        }
    }

    /// System prompt guidance for this effort level.
    #[must_use]
    pub fn system_prompt_hint(self) -> &'static str {
        match self {
            Self::Relaxed => "Be concise. Give short, direct answers. Skip detailed explanations unless asked.",
            Self::Balanced => "",
            Self::Thorough => "Be thorough. Consider edge cases, explain your reasoning, and verify your work carefully.",
        }
    }
}
...
pub struct RuntimeUiConfig {
    banner: RuntimeUiBannerConfig,
    motion: RuntimeUiMotionConfig,
    animation: RuntimeUiAnimationConfig,
    hud: RuntimeUiHudConfig,
    theme: ThemeMode,
}
...
impl RuntimeUiConfig {
    ...
    #[must_use]
    pub fn with_motion_reduced(mut self, reduced: bool) -> Self {
        self.motion = RuntimeUiMotionConfig::new(reduced);
        self
    }

    #[must_use]
    pub fn with_animation_mode(mut self, mode: RuntimeUiAnimationMode) -> Self {
        self.animation = RuntimeUiAnimationConfig::new(mode);
        self
    }

    #[must_use]
    pub fn with_hud_preset(mut self, preset: RuntimeUiHudPreset) -> Self {
        self.hud = RuntimeUiHudConfig::new(preset);
        self
    }
    ...
    #[must_use]
    pub fn banner(&self) -> &RuntimeUiBannerConfig {
        &self.banner
    }

    #[must_use]
    pub fn motion(&self) -> &RuntimeUiMotionConfig {
        &self.motion
    }

    #[must_use]
    pub fn animation(&self) -> &RuntimeUiAnimationConfig {
        &self.animation
    }

    #[must_use]
    pub fn hud(&self) -> &RuntimeUiHudConfig {
        &self.hud
    }
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        ...
        // Emberforge config paths checked first, then legacy Claw paths as fallback.
        vec![
            ConfigEntry {
            ...
            // Emberforge project config (preferred)
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".ember.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".ember").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".ember").join("settings.local.json"),
            },
            // Legacy Claw project config (fallback)
            ConfigEntry {
                source: ConfigSource::Project,
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            ui: parse_optional_ui_config(&merged_value)?,
        };
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn ui(&self) -> &RuntimeUiConfig {
        &self.feature_config.ui
    }
}
...
pub fn default_config_home() -> PathBuf {
    // Emberforge env var (preferred)
    std::env::var_os("EMBER_CONFIG_HOME")
        .map(PathBuf::from)
        // Legacy Claw env var (fallback)
        .or_else(|| std::env::var_os("CLAW_CONFIG_HOME").map(PathBuf::from))
        // Default: ~/.ember (preferred) or ~/.claw (fallback)
        .or_else(|| {
...
fn parse_optional_ui_config(root: &JsonValue) -> Result<RuntimeUiConfig, ConfigError> {
    let Some(ui_value) = root.as_object().and_then(|object| object.get("ui")) else {
        return Ok(RuntimeUiConfig::default());
    };
    let ui = expect_object(ui_value, "merged settings.ui")?;

    let banner = match ui.get("banner") {
        Some(value) => parse_ui_banner_config(value, "merged settings.ui.banner")?,
        None => RuntimeUiBannerConfig::default(),
    };
    let motion = match ui.get("motion") {
        Some(value) => parse_ui_motion_config(value, "merged settings.ui.motion")?,
        None => RuntimeUiMotionConfig::default(),
    };

    let animation = match ui.get("animation") {
        Some(value) => parse_ui_animation_config(value, "merged settings.ui.animation")?,
        None => RuntimeUiAnimationConfig::default(),
    };

    let hud = match ui.get("hud") {
        Some(value) => parse_ui_hud_config(value, "merged settings.ui.hud")?,
        None => RuntimeUiHudConfig::default(),
    };
...
fn parse_ui_banner_config(
    value: &JsonValue,
    context: &str,
) -> Result<RuntimeUiBannerConfig, ConfigError> {
    if let Some(mode) = value.as_str() {
        return Ok(RuntimeUiBannerConfig {
            mode: parse_ui_banner_mode(mode, context)?,
            ..RuntimeUiBannerConfig::default()
        });
    }

    let banner = expect_object(value, context)?;
    let mode = optional_string(banner, "mode", context)?
        .map(|value| parse_ui_banner_mode(value, &format!("{context}.mode")))
        .transpose()?
        .unwrap_or_default();
    let variant = optional_string(banner, "variant", context)?
        .map(|value| parse_ui_banner_variant(value, &format!("{context}.variant")))
        .transpose()?
        .unwrap_or_default();
...
fn parse_ui_banner_mode(
    ...
    match value {
        ...
        "auto" => Ok(RuntimeUiBannerMode::Auto),
        "off" => Ok(RuntimeUiBannerMode::Off),
        other => Err(ConfigError::Parse(format!(
            "{context}: unsupported ui.banner.mode {other}"
        ))),
    }
}

fn parse_ui_banner_variant(
    value: &str,
    context: &str,
) -> Result<RuntimeUiBannerVariant, ConfigError> {
    match value {
        "auto" => Ok(RuntimeUiBannerVariant::Auto),
        "compact" => Ok(RuntimeUiBannerVariant::Compact),
        "wide" => Ok(RuntimeUiBannerVariant::Wide),
        other => Err(ConfigError::Parse(format!(
            "{context}: unsupported ui.banner.variant {other}"
        ))),
    }
}

fn parse_ui_motion_config(
    value: &JsonValue,
    context: &str,
) -> Result<RuntimeUiMotionConfig, ConfigError> {
    let motion = expect_object(value, context)?;
    Ok(RuntimeUiMotionConfig {
        reduced: optional_bool(motion, "reduced", context)?.unwrap_or(false),
    })
}

fn parse_ui_animation_config(
    value: &JsonValue,
    context: &str,
    ...
    let mode = optional_string(animation, "mode", context)?
        .map(|value| parse_ui_animation_mode(value, &format!("{context}.mode")))
        .transpose()?
        .unwrap_or_default();
    Ok(RuntimeUiAnimationConfig::new(mode))
}

fn parse_ui_animation_mode(
    value: &str,
    context: &str,
) -> Result<RuntimeUiAnimationMode, ConfigError> {
    match value {
        "off" => Ok(RuntimeUiAnimationMode::Off),
        "intro" => Ok(RuntimeUiAnimationMode::Intro),
        "auto" => Ok(RuntimeUiAnimationMode::Auto),
        other => Err(ConfigError::Parse(format!(
            "{context}: unsupported ui.animation.mode {other}"
        ))),
    }
}

fn parse_ui_hud_config(
    value: &JsonValue,
    context: &str,
    ...
    let preset = optional_string(hud, "preset", context)?
        .map(|value| parse_ui_hud_preset(value, &format!("{context}.preset")))
        .transpose()?
        .unwrap_or_default();
...
fn parse_ui_hud_preset(
    ...
    match value {
        ...
        "focused" => Ok(RuntimeUiHudPreset::Focused),
        "full" => Ok(RuntimeUiHudPreset::Full),
        other => Err(ConfigError::Parse(format!(
            "{context}: unsupported ui.hud preset {other}"
        ))),
    }
}
