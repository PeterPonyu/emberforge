...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Vec<StreamEvent> {
        ...
        for choice in chunk.choices {
            // Buffer reasoning/thinking tokens (Ollama thinking models like qwen3, deepseek-r1).
            // Also emit as ThinkingDelta for verbose display.
            if let Some(reasoning) = choice.delta.reasoning.filter(|v| !v.is_empty()) {
    ...
    fn finish(&mut self) -> Vec<StreamEvent> {
        ...
        // Thinking model fallback: if the model put ALL output into reasoning
...
struct ChatMessage {
    ...
    /// Ollama returns thinking/reasoning in this field for thinking models.
    #[serde(default)]
...
fn build_chat_completion_request(request: &MessageRequest) -> Value {
    ...
    let mut payload = json!({
        ...
        // Keep models loaded in Ollama for 60 minutes to avoid cold-start
        // delays between turns. Ollama ignores this field; other providers
        // silently discard unknown keys per OpenAI spec.
        "keep_alive": "60m",
    });
...
fn normalize_response(
    ...
    // Use content if available; fall back to reasoning for thinking models
    // (Ollama puts qwen3/deepseek-r1 output entirely in reasoning field).
    let text = choice
        .message
        .content
        .filter(|v| !v.is_empty())
        .or_else(|| choice.message.reasoning.filter(|v| !v.is_empty()));
