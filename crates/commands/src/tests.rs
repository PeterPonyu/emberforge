...
use crate::spec::{resume_supported_slash_commands, slash_command_specs};
...
fn write_external_plugin(root: &Path, name: &str, version: &str) {
    fs::create_dir_all(root.join(".ember-plugin")).expect("manifest dir");
    fs::write(
        root.join(".ember-plugin").join("plugin.json"),
        format!(
...
fn parses_supported_slash_commands() {
    ...
    assert_eq!(
        ...
        Some(SlashCommand::Doctor { mode: None })
    );
    assert_eq!(
        SlashCommand::parse("/doctor full"),
    ...
    assert_eq!(
        ...
        Some(SlashCommand::Model {
            model: Some("list".to_string()),
        })
    );
    assert_eq!(
        SlashCommand::parse("/tasks stop agent-123"),
        Some(SlashCommand::Tasks {
            action: Some("stop agent-123".to_string()),
        })
    );
    assert_eq!(
        SlashCommand::parse("/tasks logs agent-123"),
        Some(SlashCommand::Tasks {
            action: Some("logs agent-123".to_string()),
        })
    );
    assert_eq!(
...
fn renders_help_from_shared_specs() {
    ...
    assert!(help.contains("/doctor [quick|full|status|reset]"));
...
fn parses_quoted_skill_frontmatter_values() {
    ...
    let (name, description) = crate::handlers::parse_skill_frontmatter(contents);
