...
const GITIGNORE_COMMENT: &str = "# Emberforge local artifacts";
...
pub(crate) fn render_init_ember_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# EMBER.md".to_string(),
        String::new(),
        "This file provides guidance to Emberforge (emberforge.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.ember.json`; reserve `.ember/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `EMBER.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        // Check each artifact appears with "created" status (padding varies by name length)
        assert!(rendered.contains(".ember/") && rendered.contains("created"));
        assert!(rendered.contains(".ember.json") && rendered.contains("created"));
        assert!(rendered.contains(".gitignore") && rendered.contains("created"));
        assert!(rendered.contains("EMBER.md") && rendered.contains("created"));
        assert!(root.join(".ember").is_dir());
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert!(second_rendered.contains(".ember.json") && second_rendered.contains("skipped"));
        assert!(second_rendered.contains(".gitignore") && second_rendered.contains("skipped"));
        assert!(second_rendered.contains("EMBER.md") && second_rendered.contains("skipped"));
        assert_eq!(
